...
return function(I, P, VT, ds)
	...
	if CandleExist(I,ds) then
		...
		if it.l == 1 then
			...
			if tmp.pp then tmp.p = (tmp.pp*(P-1) + 2*GetValueEX(it.p,VT,ds)) / (P+1); end
		end
		if it.l >= P then
			return tmp.p
		end
...
function ConvertValue(T,...)
	local function r(V, R) 
		...
		if V and tonumber(R) then
			if V >= 0 then 
				return math.floor(V * 10^R + 0.5) / 10^R
			else
				return math.ceil(V * 10^R - 0.5) / 10^R 
			end
		else return V end
	end
	if #{...} then
		...
		for i, v in ipairs({...}) do
			v=v and r(v * ((T and T.Multiply) or 1), (T and T.Round) or "off")
		end
		return unpack({...})
	else return nil end
end
